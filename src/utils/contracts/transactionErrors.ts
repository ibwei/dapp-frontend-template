...
const TX_ERROR_PATTERNS: { [key: string]: ErrorPattern[] } = {
  [NOT_ENOUGH_FUNDS]: [{ msg: 'Insufficient Gas' }, { msg: 'failed to execute call with revert code InsufficientGasFunds' }],
  ...
  [NETWORK_CHANGED]: [{ msg: 'Network error' }],
  [RPC_ERROR]: [
  ...
  for (const [type, patterns] of Object.entries(TX_ERROR_PATTERNS)) {
    for (const pattern of patterns) {
      ...
      if (matchCode || matchMessage) {
        return [pattern.msg, type, ex.data]
      }
    }
  }
